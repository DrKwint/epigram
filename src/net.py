...
class ENN(nnx.Module):
    ...
    def epinet(self, full_phi: jax.Array, z: jax.Array):
        epi_hidden = jax.nn.relu(self.epi_fc1(jnp.concatenate([full_phi, z], axis=-1)))
        return self.epi_out(epi_hidden)

    def prior(self, full_phi: jax.Array, z: jax.Array):
    ...
    def propagate_star_set(self, star: Star) -> list[Star]:
        ...
        # [x, a] corresponds to the first (x_dim + a_dim) outputs of star.transform
        ...
        stars_phi = star_phi_pre.map_relu()
        ...
        for s_phi in tqdm(stars_phi):
            ...
            for s_epi in tqdm(stars_epi):
                ...
                stars_prior = Star(s_epi.input_set, T_pre_prior).map_relu()

                for s_prior in tqdm(stars_prior):
                    ...
                    # result_stars.append(Star(s_prior.input_set, Affine(A_final, b_final)))
